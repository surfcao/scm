...
for id=1:nDir
    Out.struct{id} = nan(numel(lagDist{id}),5,nMeas);
end
...
for id=1:nDir
    for il=1:nLag(id)
        ...
        for im=1:nMeas
            ...
            Out.struct{id}(il,1,im) = lagDist{id}(il);
            Out.struct{id}(il,2,im) = outMeas;
            Out.struct{id}(il,3,im) = MeasSpecs(im,1);
            Out.struct{id}(il,4,im) = MeasSpecs(im,2);
            Out.struct{id}(il,5,im) = MeasSpecs(im,3);
        end
    end
end
