function gam = covmodgauss(modPars,lagDist)
% Gaussian model semivariogram values for given distances (isotropic)
%
%% DESCRIPTION: vmodgauss.m
% Function to compute semivariogram values at given distances in lagDist,
% from an isotropic GAUSSIAN model with parameters in modPars.
% NOTE: The 1st entry in modPars pertains to the nugget effect.
%
%% SYNTAX:
%   gam = vmodgauss(modPars,lagDist);
%
%% INPUTS:
%   modPars      = (1 x 3) array [nugg sill range] with isotropic 
%                  semivariogram model parameters:
%                       nugg  = partial sill for nugget effect
%                       sill  = partial sill for Gaussian model
%                       range = practical range for Gaussian model  
